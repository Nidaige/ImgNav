...
from bosdyn.api import trajectory_pb2, image_pb2, robot_state_pb2, local_grid_pb2, geometry_pb2
...
#--username user --password 0hucscziz28r 192.168.80.3
...
class SpotInterface:
    ...
    TRAJECTORY_CMD_TIMEOUT = 15.0  # [seconds]
    ...
    def orientation_cmd_srv(self, orientation):
        orr = [0,0,0]
        for pose in orientation.orientation.poses:
            orr = pose.orientation

            footprint_R_body = bosdyn.geometry.EulerZXY(yaw= orr.z, roll= orr.x, pitch= orr.y)
            cmd = RobotCommandBuilder.synchro_stand_command(footprint_R_body=footprint_R_body)
            self.command_client.robot_command(cmd,end_time_secs=time.time() + self.VELOCITY_CMD_DURATION
            )
            rospy.loginfo("Robot standing twisted.")
...
#        
#        body_control1 = spot_command_pb2.BodyControlParams(base_offset_rt_footprint=traj)
#        
#        
        vel_desired = 0.5
        
        speed_limit = geometry_pb2.SE2VelocityLimit(
        max_vel=geometry_pb2.SE2Velocity(linear=geometry_pb2.Vec2(x=vel_desired, y=vel_desired), angular=.25))
        
        obstacles = spot_command_pb2.ObstacleParams(obstacle_avoidance_padding=0.3)
        
#        body_control = body_control1
        
        mobility_params = spot_command_pb2.MobilityParams(vel_limit=speed_limit, obstacle_params=obstacles, #body_control=body_control,
        ...
        self.robot_state_client.get_robot_state().kinematic_state.transforms_snapshot)
            ...
            body_tform_goal = math_helpers.SE3Pose(x, y, z, rot = math_helpers.Quat(q.w, q.x, q.y, q.z))
        ...
        rospy.loginfo(
            "Robot trajectory cmd sent: x=${},y=${}".format(x, y))
          
        return[]   
      
    def trajectory_cmd_srv_global(self, globaltrajectory):
        x = 0
        y = 0
        z = 0
        
        for pose in globaltrajectory.globalwaypoints.poses:
            x = pose.position.x
            y = pose.position.y
            z = pose.position.z
            q = pose.orientation
            
            rot = math_helpers.Quat(q.w, q.x, q.y, q.z)
            frame = VISION_FRAME_NAME
    ...
    def is_final_state(self, goal):
		    ...
		    if ((x_dist < self.x_goal_tolerance) and (y_dist < self.y_goal_tolerance) and (angle < self.angle_goal_tolerance)):
		        rospy.loginfo_once('at goal')
		        return True
		    rospy.loginfo_once('not at goal')
		    return False
    ...
    def start_spot_ros_interface(self):
        ...
        rospy.Service("global_trajectory_cmd", spot_ros_srvs.srv.globalTrajectory, self.trajectory_cmd_srv_global)
        rospy.Service("velocity_cmd", spot_ros_srvs.srv.Velocity, self.velocity_cmd_srv)
        rospy.Service("orientation_cmd", spot_ros_srvs.srv.Orientation, self.orientation_cmd_srv)
...
#        #Publish metrics
#        robot_position_pub = rospy.Publisher('Robot_position', spot_ros_msgs.msg.RobotPos, queue_size = 20)
        ...
        try:
            with bosdyn.client.lease.LeaseKeepAlive(self.lease_client), bosdyn.client.estop.EstopKeepAlive(
                ...
                while not rospy.is_shutdown():
                    ...
                    ## Publish robot metrics
    
#    
#    
#                    robot_pos = get_vision_tform_body(
#        self.robot_state_client.get_robot_state().kinematic_state.transforms_snapshot)
#                    robot_position_pub.publish(robot_pos)
...
if __name__ == '__main__':
    ...
    # parse known args only due to the extra arguments from ROSlaunch
